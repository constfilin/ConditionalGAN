...
def conv2d(input_,output_dim,k_h=3,k_w=3,d_h=2,d_w=2,name="conv2d"):
    with tf.variable_scope(name):
        weights = tf.get_variable('w',[k_h, k_w, input_.get_shape()[-1],output_dim],initializer=variance_scaling_initializer())
        conv    = tf.nn.conv2d(input_,weights,strides=[1,d_h,d_w,1],padding='SAME')
        biases  = tf.get_variable('biases', [output_dim], initializer=tf.constant_initializer(0.0))
        conv    = tf.reshape(tf.nn.bias_add(conv, biases),conv.get_shape())
        return conv, weights

def de_conv(input_,output_shape,k_h=3,k_w=3,d_h=2,d_w=2,stddev=0.02,name="deconv2d",with_w=False,initializer=variance_scaling_initializer()):
    with tf.variable_scope(name):
